...
export const legacyAdaptationSchema = obj({
  summary: text,
...
export const adaptationSchema = obj({
  ...
  candidates: list(obj({
    id: text,
    title: text,
    gameplay: { type: "string", maxLength: 150 },
    media_usage: list(usage),
    meme_interest: text,
    references: list(reference),
  })),
...
export function checkAdaptation(value, count, research, assets, schema = adaptationSchema) {
  ...
  for (const c of value.candidates) {
    if (
      (schema === legacyAdaptationSchema
        ? ["title", "hook", "rules", "controls", "meme_interest", "minimum_implementation"]
        : ["title", "gameplay", "meme_interest"]
      ).some((k) => !c[k].trim())
    )
      throw new Error("选题内容不完整");
    if (
      !c.references.length ||
      c.references.some(
        (ref) =>
