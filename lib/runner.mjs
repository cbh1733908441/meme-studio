...
export function eventMessage(e) {
  ...
  if (type === 'agent_message' && e.item?.text && !e.item.text.trim().startsWith('{')) return e.item.text.slice(0, 220);
  ...
  if (e.type === 'error' || e.type === 'turn.failed') {
    ...
    return 'Codex 执行异常：' + raw.replace(/Bearer\s+\S+|sk-[A-Za-z0-9_-]+/g, '[已隐藏]').slice(0, 220);
  }
}
export function runCodex({
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      if (timedOut)
        return reject(
          new Error("本阶段运行超过 30 分钟，已停止；可保留资料重试。"),
        );
      if (stopped || signal?.aborted) return reject(new Error("已取消"));
      if (code !== 0)
        return reject(
          new Error(
            `Codex CLI 退出（${code}）。${stderr.replace(/Bearer\s+\S+|sk-[A-Za-z0-9_-]+/g, "[已隐藏]").slice(-1600)}`,
          ),
        );
      try {
